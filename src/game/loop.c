...
extern void processGameStateMachine(void);       // Original: func_001bc2a0 at 0x1bc200
...
extern void func_001aefe0(int value);            // Original: func_001aefe0 at 0x1aefd0
...
extern void func_0019d450(int index, int param); // Original: func_0019d450 at 0x19d440

// Global state address (original: 0x310000 + 0x3884 = 0x313884)
#define GLOBAL_STATE_ADDR 0x00313884
static uint16_t* g_gameState = (uint16_t*)GLOBAL_STATE_ADDR;
 ...
 * @address 0x001bbfb0
 * @description Main game update loop. Checks global state and either runs
 *              state machine (if state == 4) or performs rendering/display
 *              operations with text and UI elements.
 * @windows_compatibility high
 */
void updateGameLoop(void) {
    // Load current game state from global
    uint16_t currentState = *g_gameState;

    // Early exit if state is 0
    if (currentState == 0) {
        return;
    }

    // If state is 4, run the state machine
    if (currentState == 4) {
        processGameStateMachine();  // Original: func_001bc2a0 at 0x1bc200
        return;
    }

    // Otherwise, perform rendering/display logic
    ...
    // Checking resource ID 14 to verify the resource initialized in state machine is ready
    ...
    func_001aefe0(0xd);                   // Original: func_001aefe0 at 0x1aefd0
    func_001aefe0(0x5f);                  // Original: func_001aefe0 at 0x1aefd0
    func_001aefe0(0x60);                  // Original: func_001aefe0 at 0x1aefd0
    func_001aefe0(0x63);                  // Original: func_001aefe0 at 0x1aefd0
    func_001aefe0(0x6c);                  // Original: func_001aefe0 at 0x1aefd0

    // Call function at index 1
    func_0019d450(1, checkResult);        // Original: func_0019d450 at 0x19d440

    func_001aefe0(checkResult);           // Original: func_001aefe0 at 0x1aefd0
    ...
    // If state is 3, perform additional rendering operations
    if (currentState != 3) {
        return;
    }

    // Get base address of text array
